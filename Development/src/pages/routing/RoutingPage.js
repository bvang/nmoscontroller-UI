...
const RoutingListAudio = props => {
    ...
    const { data: flowaudioData } = useGetList({
        ...props,
        resource: 'flows',
        filter,
    });
    ...
    const handleClickFlowAudio = async (manifestHref, id) => {
        const parts = manifestHref.split('/');
        const baseUrl = parts.slice(0, 3).join('/') + '/';
        const URL = `${baseUrl}x-nmos/connection/v1.1/single/senders/${id}/transportfile/`;
        fetch(URL)
    ...
    function createMatchingCardsSenderFlowAudio(senderData, flowaudioData) {
        const matchingItems = [];
        senderData.forEach(item1 => {
    ...
    function createMatchingCardsReceiverFlowAudio(receiverData, devicesData) {
        const matchingItems = [];
        receiverData.forEach(item1 => {
            devicesData.forEach(item2 => {
                if (item1.device_id === item2.id) {
                    const data = item2.controls;
                    if (item1.format === 'urn:x-nmos:format:audio') {
                        const desiredType = 'urn:x-nmos:control:sr-ctrl/v1.1';
                        const desiredHref = data.find(
                            obj => obj.type === desiredType
                        )?.href;
                    ...
                    <List>
                        ...
                        <TableContainer>
                            <Table>
                                ...
                                <TableBody>
                                    {createMatchingCardsSenderFlowAudio(
                                        senderData,
                                        flowaudioData
                                    ).map((card, index) => (
                                        <TableRow key={index}>
                    ...
                    <List>
                        ...
                        <TableContainer>
                            <Table>
                                ...
                                <TableBody>
                                    {createMatchingCardsReceiverFlowAudio(
                                        receiverData,
                                        devicesData
                                    ).map((card, index) => (
                                        <TableRow key={index}>
...
const RoutingListData = props => {
    ...
    const { data: flowdataData } = useGetList({
        ...props,
        resource: 'flows',
        filter,
    });
    ...
    const handleClickFlowData = async (manifestHref, id) => {
        const parts = manifestHref.split('/');
        const baseUrl = parts.slice(0, 3).join('/') + '/';
        const URL = `${baseUrl}x-nmos/connection/v1.1/single/senders/${id}/transportfile/`;
        fetch(URL)
    ...
    function createMatchingCardsSenderFlowData(senderData, flowdataData) {
        const matchingItems = [];
        senderData.forEach(item1 => {
            flowdataData.forEach(item2 => {
                if (item1.flow_id === item2.id) {
                    const data = item2.format;
                    if (item2.format === 'urn:x-nmos:format:data') {
    ...
    function createMatchingCardsReceiverFlowData(receiverData, devicesData) {
        const matchingItems = [];
        receiverData.forEach(item1 => {
            devicesData.forEach(item2 => {
                if (item1.device_id === item2.id) {
                    const data = item2.controls;
                    if (item1.format === 'urn:x-nmos:format:data') {
                        console.log(
                    ...
                    <List>
                        ...
                        <TableContainer>
                            <Table>
                                ...
                                <TableBody>
                                    {createMatchingCardsSenderFlowData(
                                        senderData,
                                        flowdataData
                                    ).map((card, index) => (
                                        <TableRow key={index}>
                    ...
                    <List>
                        ...
                        <TableContainer>
                            <Table>
                                ...
                                <TableBody>
                                    {createMatchingCardsReceiverFlowData(
                                        receiverData,
                                        devicesData
                                    ).map((card, index) => (
                                        <TableRow key={index}>
